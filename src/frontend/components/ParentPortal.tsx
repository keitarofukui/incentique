      ...
      <div className="flex items-center gap-2 border-b border-slate-800/80 pb-2 overflow-x-auto scrollbar-none">
        ...
        >
          ...
          {claimedWishes.length > 0 && (
            <span className="bg-rose-500 text-white text-xs font-black px-1.5 py-0.5 rounded-full animate-bounce">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {isCash && (
                              <span className="text-xs font-bold text-emerald-300 bg-emerald-500/20 px-2 py-0.5 rounded-full border border-emerald-500/40">
                                💵 現金還元 (7掛け)
                              </span>
                            )}
                          </div>
                ...
                <>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
                    ...
                    <div className="flex items-center gap-3 text-xs shrink-0 bg-slate-950/60 p-2.5 rounded-xl border border-slate-800">
                      <div>
                        <span className="text-slate-400 block text-xs">引き落とし累計</span>
                        <strong className="text-amber-400 font-mono text-sm">{totalDeducted.toLocaleString()} pt</strong>
                      </div>
                      {totalCash > 0 && (
                        <div className="border-l border-slate-800 pl-3">
                          <span className="text-slate-400 block text-xs">現金還元累計</span>
                          ...
                          <div key={item.id} className="p-4 flex items-start justify-between gap-3 hover:bg-slate-900/40 transition-colors">
                            <div className="min-w-0 space-y-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className="text-xs font-bold text-amber-300 bg-amber-500/10 border border-amber-500/30 px-2 py-0.5 rounded-full">
                                  {item.user_name || 'メンバー'}
                                </span>
                                {item.item_type === 'cash' && (
                                  <span className="text-xs font-bold text-emerald-300 bg-emerald-500/10 border border-emerald-500/30 px-2 py-0.5 rounded-full">
                                    💵 現金受領
                                  </span>
                                )}
                              ...
                              {item.product_url && (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="inline-flex items-center gap-1 text-xs font-bold text-orange-300 hover:text-orange-200 underline underline-offset-2"
                                >
                                  🛒 購入・参照ページ
                                </a>
                              )}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="p-4 rounded-2xl bg-indigo-950/40 border border-indigo-500/30 space-y-2">
                  ...
                  <div className="text-xs text-slate-400">1アクション以上の日常記録</div>
            ...
            <div key={m.id} className="bg-slate-900/60 p-4 rounded-2xl border border-slate-800 space-y-2">
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-2">
                ...
                <div className="flex items-center gap-1">
                  <span className="text-xs text-slate-400">基準:</span>
                  <input
                    type="number"
                    step={5}
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-300 focus:outline-none focus:border-emerald-400 font-mono"
              />
            </div>
          ))
        )}
      ...
      <form onSubmit={handleAddMenu} className="bg-slate-950/80 p-4 rounded-2xl border border-amber-500/30 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
          <div className="sm:col-span-2">
            <label className="block text-xs text-slate-400 mb-1">アイコン</label>
            <input
              type="text"
              value={newIcon}
          ...
          <div className="sm:col-span-5">
            <label className="block text-xs text-slate-400 mb-1">家事メニュー名</label>
            <input
              type="text"
          ...
          <div className="sm:col-span-3">
            <label className="block text-xs text-slate-400 mb-1">獲得ポイント (pt)</label>
            <input
              type="number"
              min={1}
              max={1000}
